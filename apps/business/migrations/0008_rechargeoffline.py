# Generated by Django 2.2.6 on 2020-03-01 18:07

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('business', '0007_auto_20200301_1536'),
    ]

    operations = [
        migrations.CreateModel(
            name='RechargeOffline',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('remark', models.TextField(blank=True, max_length=40, null=True, verbose_name='审核备注')),
                ('state', models.IntegerField(blank=True, choices=[(0, '未审核'), (1, '审核通过'), (2, '审核拒绝')], default=0, null=True, verbose_name='审核状态')),
                ('auditor', models.CharField(blank=True, max_length=10, null=True, verbose_name='审核人')),
                ('applyTime', models.DateTimeField(auto_now_add=True, verbose_name='申请时间')),
                ('audiTime', models.DateTimeField(blank=True, null=True, verbose_name='审核时间')),
                ('tradeCode', models.CharField(blank=True, max_length=50, null=True, verbose_name='交易号')),
                ('amount', models.DecimalField(decimal_places=4, default=0.0, max_digits=18, verbose_name='充值金额')),
                ('tradeTime', models.DateTimeField(blank=True, null=True, verbose_name='充值时间')),
                ('note', models.TextField(blank=True, max_length=40, null=True, verbose_name='充值说明')),
                ('applier', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='申请人')),
                ('bankInfo', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='business.PlatformBankInfo', verbose_name='平台账户')),
            ],
            options={
                'verbose_name': '线下充值表单',
                'verbose_name_plural': '线下充值表单',
            },
        ),
    ]
